...
const offers = [{
  ...
  priceNote: "12 meses",
  discount: "40% OFF",
  includes: ["12 meses: NZT Dia + Tarde + Noite", "Smart Ring (sensor contínuo)", "Plataforma Premium", "Correlações inteligentes (exclusivo)", "Insights de AI (exclusivo)", "Entrega expressa grátis"],
...
export function ProductCard() {
  return <section id="produto" className="py-24 bg-gradient-to-b from-slate-950 to-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-20">
          {offers.map(offer => <div key={offer.id} className={`relative rounded-2xl p-1 ${offer.premium ? "bg-gradient-to-br from-fuchsia-600 via-violet-600 to-indigo-600" : offer.highlight ? "bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600" : "bg-slate-700/50"}`}>
              {/* Glow for premium/highlight */}
              {(offer.premium || offer.highlight) && <div className="absolute inset-0 bg-gradient-to-r from-violet-500/30 via-purple-500/30 to-fuchsia-500/30 blur-xl rounded-2xl" />}

              <div className={`relative bg-slate-900 rounded-xl p-6 h-full flex flex-col ${offer.premium ? "border border-fuchsia-500/20" : ""}`}>
                {/* Tier Badge */}
                <div className="mb-4">
                  <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full uppercase tracking-wider ${offer.premium ? "bg-gradient-to-r from-fuchsia-500 to-violet-500 text-white" : offer.highlight ? "bg-gradient-to-r from-amber-500 to-orange-500 text-white" : "bg-slate-700 text-slate-300"}`}>
                ...
                {/* Name & Description */}
                <h3 className={`text-2xl font-bold mb-2 ${offer.premium ? "bg-gradient-to-r from-fuchsia-400 via-violet-400 to-indigo-400 bg-clip-text text-transparent" : "text-white"}`}>
                  {offer.name}
                </h3>
                ...
                {/* Pricing */}
                <div className="mb-6">
                  {offer.originalPrice && <div className="flex items-center gap-2 mb-1">
                ...
                <ul className="space-y-3 mb-8 flex-grow">
                  {offer.includes.map((item, index) => <li key={index} className="flex items-start gap-3 text-sm">
                      <Check className={`w-4 h-4 mt-0.5 ${offer.premium ? "text-fuchsia-400" : offer.highlight ? "text-violet-400" : "text-emerald-400"}`} />
                ...
                {/* CTA */}
                <Link to={offer.link}>
                  <Button className={`w-full py-6 text-base font-semibold rounded-xl transition-all duration-300 ${offer.premium ? "bg-gradient-to-r from-fuchsia-600 via-violet-600 to-indigo-600 hover:from-fuchsia-700 hover:via-violet-700 hover:to-indigo-700 text-white shadow-lg shadow-fuchsia-500/25 hover:shadow-xl hover:shadow-fuchsia-500/30 hover:scale-[1.02]" : offer.highlight ? "bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-600 hover:from-violet-700 hover:via-purple-700 hover:to-fuchsia-700 text-white shadow-lg shadow-purple-500/25 hover:shadow-xl hover:shadow-purple-500/30 hover:scale-[1.02]" : "bg-slate-700 hover:bg-slate-600 text-white"}`}>
                    {offer.cta}
                  </Button>
                </Link>

                {/* Security note for premium */}
                {offer.premium && <p className="text-xs text-slate-500 mt-4 flex items-center justify-center gap-1.5">
        ...
        <div className="text-center mb-8">
          <p className="text-sm text-slate-300">Conheça cada produto do ciclo cognitivo</p>
        </div>

        <div className="grid md:grid-cols-3 gap-4">
          {individualProducts.map(product => <div key={product.id} className={`p-5 rounded-xl bg-slate-800/30 border ${product.borderColor} flex items-center gap-4`}>
