...
export function MetricCard({ metric }: MetricCardProps) {
  return (
    <div className="vyr-card-graphite p-2.5 sm:p-4">
      <div className="text-xs sm:text-sm text-vyr-gray-400 truncate font-mono">{metric.key}</div>
      <div className="text-lg sm:text-2xl font-semibold text-vyr-white mt-0.5 sm:mt-1">
        {Number.isFinite(metric.value) ? metric.value : "--"}
        <span className="text-xs sm:text-sm text-vyr-gray-500 ml-1 sm:ml-2">{metric.unit ?? ""}</span>
      </div>
      {typeof metric.confidence === "number" && (
        <div className="text-[10px] sm:text-xs text-vyr-gray-500 mt-1 sm:mt-2">
...
export function IndexCard({ label, value, icon }: IndexCardProps) {
  return (
    <div className="vyr-card-graphite p-2.5 sm:p-4">
      <div className="flex items-center gap-1 sm:gap-2 text-[10px] sm:text-xs text-vyr-gray-400 font-mono">
